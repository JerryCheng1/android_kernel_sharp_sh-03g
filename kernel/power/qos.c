...
enum {
	...
	SH_DEBUG_QOS_REQUEST_CHANGE    = 1U << 3,
};
			...
			int pm_qos_class, s32 value)
{
...
#ifdef CONFIG_SHSYS_CUST_DEBUG
			if(sh_debug_mask & SH_DEBUG_QOS_REQUEST_CHANGE){
				cpumask_scnprintf(buf, sizeof(buf), &req->cpus_affine);
				pr_info("QoS: [ChangeRequest] before_type PM_QOS_REQ_AFFINE_CORES(%d), changed_type %d, cpus_affine 0x%s\n",(int)PM_QOS_REQ_AFFINE_CORES, (int)req->type, buf);
			}
	...
	case PM_QOS_REQ_ALL_CORES:
		...
		if(sh_debug_mask & SH_DEBUG_QOS_REQUEST_CHANGE){
			cpumask_scnprintf(buf, sizeof(buf), &req->cpus_affine);
			pr_info("QoS: [ChangeRequest] before_type PM_QOS_REQ_ALL_CORES(%d), changed_type %d, cpus_affine 0x%s\n", (int)PM_QOS_REQ_ALL_CORES, (int)req->type, buf);
		}
#endif /* CONFIG_SHSYS_CUST_DEBUG */
#else
		cpumask_setall(&req->cpus_affine);
