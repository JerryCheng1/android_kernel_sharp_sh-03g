...
static int escore_runtime_suspend(struct device *dev)
{
	...
	vdd_core = regulator_get(escore->dev, "pm8994_l25");
	...
	if (IS_ERR_OR_NULL(vdd_core)) {
		...
		enabled = regulator_is_enabled(vdd_core);

		if (enabled) {
			ret = regulator_set_mode(vdd_core, REGULATOR_MODE_IDLE);
			dev_dbg(dev, "%s() regulator_set_mode REGULATOR_MODE_IDLE %d\n", __func__, ret);
			if (ret) {
				dev_err(dev, "%s() regulator_set_mode failed %d\n", __func__, ret);
				ret = 0;
			}
		} else {
			pr_err("%s(): pm8994_l25 is disable\n", __func__);
		}
...
static int escore_runtime_resume(struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(vdd_core)) {
		...
		if (enabled) {
			...
			dev_dbg(dev, "%s() regulator_set_mode REGULATOR_MODE_NORMAL %d\n", __func__, ret);
			if (ret) {
				dev_err(dev, "%s() regulator_set_mode failed %d\n", __func__, ret);
				ret = 0;
			}
...
#ifndef CONFIG_SH_AUDIO_DRIVER /* 14-061 */
