...
int sc_pinctrl_init(void)
{
	...
	pinctrl = devm_pinctrl_get(&sbdev_gol->dev);
	...
	gpio_state_active = pinctrl_lookup_state(pinctrl, "aud_active");
...
int sc_extclk_gpio_init(void)
{
	int ret = 0;
	int extclk_gpio = 0;
	dev_dbg(&sbdev_gol->dev, "%s(): Entry\n", __func__);

	extclk_gpio = of_get_named_gpio(sbdev_gol->dev.of_node,
		"adnc,extclk-gpio", 0);
	if (extclk_gpio < 0) {
		dev_err(&sbdev_gol->dev, "%s(): get extclk_gpio failed\n", __func__);
		ret = extclk_gpio;
		return -EINVAL;
	}
	dev_dbg(&sbdev_gol->dev, "%s(): extclk_gpio = %d\n", __func__, extclk_gpio);
...
int sc_clock_ctrl(int sw)
{
	...
	if (aud_ext_clk == NULL) {
		...
		aud_ext_clk = clk_get(&sbdev_gol->dev, "aud_cam_clk");
...
static int escore_slim_probe(struct slim_device *sbdev)
{
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-061 */
	{
		...
		rc = sc_pinctrl_init();
		if (rc) {
			dev_err(&sbdev->dev, "%s(): pinctrl init failed : %d\n",
				__func__, rc);
			goto out;
		}
		sc_extclk_gpio_init();
		if (rc) {
			dev_err(&sbdev->dev, "%s(): extclk gpio init failed : %d\n",
				__func__, rc);
			goto out;
		}
		sc_clock_ctrl(1);
