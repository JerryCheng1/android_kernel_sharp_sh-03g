...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-063 */
#include "sharp/sh_smem.h"
#include "sharp/shdiag_smd.h"
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 14-063 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-069 */
#include <linux/sched.h>
#include <linux/kthread.h>
#include <linux/err.h>
#include <sharp/shdisp_kerl.h>

#define AUD_LED_DELAY 100

static struct task_struct *led_task = NULL;
static int aud_led_control = 0;
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 14-069 */
...
static int es705_slim_rx_port_to_ch[ES_SLIM_RX_PORTS] = {
		152, 153, 154, 155, 134, 135, 136
};
static int es705_slim_tx_port_to_ch[ES_SLIM_TX_PORTS] = {
		156, 146, 147, 144, 145, 157, 158
};
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 14-061 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-063 */
void escore_bootmode_init(void) {
	sharp_smem_common_type * p_smem = 0;

	p_smem = sh_smem_get_common_address();
	if(p_smem != 0) {
		escore_priv.shdiag_BootMode =  p_smem->shdiag_BootMode;
		pr_err("%s shdiab boot mode:%u\n", __func__, escore_priv.shdiag_BootMode);
	} else {
		pr_err("%s get smem error\n", __func__);
	}
}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 14-063 */
					...
					const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "on", 2) && (escore_priv.pdata && escore_priv.pdata->esxxx_clk_cb)) {
		ret = escore_priv.pdata->esxxx_clk_cb(1);
	} else if (!strncmp(buf, "off", 3) && (escore_priv.pdata && escore_priv.pdata->esxxx_clk_cb)) {
		//wait for pm_runtime_set_autosuspend_delay() time end
		if (escore_priv.shdiag_BootMode != D_SHDIAG_BOOT_NORMAL)
			msleep(550);
		ret = escore_priv.pdata->esxxx_clk_cb(0);
	}

	return ret>=0? count: ret;
}

static DEVICE_ATTR(clock_on, 0666, es705_clock_on_show, es705_clock_on_set);
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 14-063 */
...
static ssize_t es705_diag_check_show(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	int ret = -1;
	ret = es705_diag_check();

	pr_info("%s ret:%d probe_check:%d\n", __func__, ret, probe_check);

	if (ret >= 0) {
		ret = probe_check <<1 | 1<<0;
	} else {
		ret = probe_check <<1 | 0<<0;
	}
...
static DEVICE_ATTR(check, 0444, es705_diag_check_show, NULL);
/* /sys/devices/soc.0/fe12f000.slim/earSmart-codec-gen0/check */
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 14-063 */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-069 */
static void aud_shdisp_tri_led_set_color(char red, char green, char blue)
{
...
static int aud_led_thread(void *data)
{
	while(1) {
		...
		if (escore_priv.preset || Preset_array[0]) {
			aud_shdisp_tri_led_set_color(255, 255, 0);
		} else {
			aud_shdisp_tri_led_set_color(0, 0, 0);
		}

		schedule_timeout(msecs_to_jiffies(AUD_LED_DELAY));
	}
...
static int es705_get_aud_led(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = aud_led_control;

	return 0;
}

static int es705_put_aud_led(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	unsigned int value;
	int rc = 0;
	...
	value = ucontrol->value.integer.value[0];
	pr_err("%s: value = %d, AUD_LED_DELAY = %d\n", __func__, value, AUD_LED_DELAY);

	if (value && !aud_led_control) {
		if (!led_task) {
			led_task = kthread_create(aud_led_thread, NULL, "audled");
			if(IS_ERR(led_task)) {
				pr_err("%s: Create new kernel thread failed\n", __func__);
				rc = PTR_ERR(led_task);
				led_task = NULL;
				return rc;
			}
			wake_up_process(led_task);
			aud_led_control = value;
			pr_err("%s: thread started %d\n", __func__, value);
		}
	} else if (!value && aud_led_control) {
		if (led_task) {
			kthread_stop(led_task);
			aud_led_control = value;
			led_task = NULL;
			pr_err("%s: thread stoped %d\n", __func__, value);
		}
	}
...
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 14-061 */
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-069 */
	SOC_SINGLE_EXT("AUDLED",
		       ES705_PRESET, 0, 65535, 0, es705_get_aud_led,
		       es705_put_aud_led),
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 14-069 */
};
...
#else
	const char *fw_filename = "Sharp_A1285_CVQ_NS_Release_MCFFillIn_STREAM.bin";
	const char *vs_filename = "audience-es705-vs.bin";
	const char *bkg_filename = "adnc_cvq_detection_bkg_w_hdrs.bin";
#endif
#else /* CONFIG_SH_AUDIO_DRIVER */ /* 14-061 */
