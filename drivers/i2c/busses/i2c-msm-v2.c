...
#define I2CDB_ERR(x, ...) do {\
	if (sh_debug_i2c & SH_DEBUG_ERROR) {\
		printk("[BSP-I2C]:%s() "x, __func__, ##__VA_ARGS__);\
	}\
} while (0)

#define I2CDB_INFO(x, ...) do {\
	if (sh_debug_i2c & SH_DEBUG_NORMAL) {\
		printk("[BSP-I2C]:%s() "x, __func__, ##__VA_ARGS__);\
	}\
} while (0)

#define I2CDB_SUSRESU(x, ...) do {\
	if (sh_debug_i2c & SH_DEBUG_SUSRESU) {\
		printk("[BSP-I2C]:%s() "x, __func__, ##__VA_ARGS__);\
	}\
} while (0)

#define I2CDB_COM(x, ...) do {\
	if (sh_debug_i2c & SH_DEBUG_COM) {\
		printk("[BSP-I2C]:%s() "x, __func__, ##__VA_ARGS__);\
	}\
} while (0)

#define I2CDB_GPIO(x, ...) do {\
	if (sh_debug_i2c & SH_DEBUG_GPIO) {\
		printk("[BSP-I2C]:%s() "x, __func__, ##__VA_ARGS__);\
	}\
} while (0)
#define I2CDB_IRQ(x, ...) do {\
...
static int i2c_msm_qup_poll_bus_active_unset(struct i2c_msm_ctrl *ctrl)
{
	...
	int    ret      = 0;
	size_t read_cnt = 0;
#if defined( CONFIG_I2C_CUST_SH_CLOCK_STRETCH )
	ulong min_sleep_usec;
#endif /* #if defined( CONFIG_I2C_CUST_SH_CLOCK_STRETCH ) */
...
poll_active_end:
	...
	I2CDB_INFO("min_sleep_usec=%lu\n",min_sleep_usec);
	usleep_range(min_sleep_usec, min_sleep_usec * 10);
#endif /* #if defined( CONFIG_I2C_CUST_SH_CLOCK_STRETCH ) */


	I2CDB_INFO("No problem(end) ret=%d read_cnt=%zu\n",ret,read_cnt);
	return ret;
}
