...
#define SHGRIP_AET_TIMEOUT_US				(2*1000*1000)
#define SHGRIP_POLL_WAIT_US_FOR_AET			(100*1000)
#endif /* SHGRIP_FACTORY_MODE_ENABLE */
...
/* shgrip_cmd_ch0sr                                                        */
/* ------------------------------------------------------------------------- */
static int shgrip_cmd_ch0sr(unsigned char *rbuf)
{
	int ret = 0;
	unsigned char buf = 0;
	...
	ret = shgrip_spi_transfer_read_block(SHGRIP_CMD_CH0SR, &buf, 1);
	if (ret) {
		SHGRIP_ERR("SHGRIP_CMD_CH0SR failed. ret:%d\n", ret);
		return ret;
	}
	
	*rbuf = buf;

	SHGRIP_DBG("CH0SR:%02x\n", buf);
...
/* shgrip_cmd_ch4sr                                                        */
/* ------------------------------------------------------------------------- */
static int shgrip_cmd_ch4sr(unsigned char *rbuf)
{
	int ret = 0;
	unsigned char buf = 0;
	...
	ret = shgrip_spi_transfer_read_block(SHGRIP_CMD_CH4SR, &buf, 1);
	if (ret) {
		SHGRIP_ERR("SHGRIP_CMD_CH4SR failed. ret:%d\n", ret);
		return ret;
	}

	*rbuf = buf;
	
	SHGRIP_DBG("CH4SR:%02x\n", buf);
...
#ifdef SHGRIP_FACTORY_MODE_ENABLE
/* ------------------------------------------------------------------------- */
/* shgrip_seq_grip_aet_check                                                 */
/* ------------------------------------------------------------------------- */
static int shgrip_seq_grip_aet_check(void)
{
	int ret = 0;
	unsigned char buf = 0;
	int time = SHGRIP_AET_TIMEOUT_US;

	SHGRIP_DBG("start\n");
	
	time = SHGRIP_AET_TIMEOUT_US;
	while(1)
	{
		shgrip_sys_delay_us(SHGRIP_POLL_WAIT_US_FOR_AET);
		ret = shgrip_cmd_ch0sr(&buf);
		if (ret) {
			SHGRIP_ERR("shgrip_cmd_ch0sr err. ret=%d\n", ret);
			break;
		}
		if((buf & BIT3)){
			if (!(time-=SHGRIP_POLL_WAIT_US_FOR_AET)) {
				SHGRIP_ERR("AET is timeout\n");
				break;
			}
		}
		else{
	...
	{
		shgrip_sys_delay_us(SHGRIP_POLL_WAIT_US_FOR_AET);
		ret = shgrip_cmd_ch4sr(&buf);
		if (ret) {
			SHGRIP_ERR("shgrip_cmd_ch4sr err. ret=%d\n", ret);
			break;
		}
		if((buf & BIT3)){
			if (!(time-=SHGRIP_POLL_WAIT_US_FOR_AET)) {
...
#ifdef SHGRIP_FACTORY_MODE_ENABLE
	ret = shgrip_seq_grip_aet_check();
	if (ret) {
		SHGRIP_ERR("shgrip_seq_grip_aet_check err. ret=%d\n", ret);
		return GRIP_RESULT_FAILURE;
	}
#endif /* SHGRIP_FACTORY_MODE_ENABLE */
