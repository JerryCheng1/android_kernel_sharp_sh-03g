...
#define SHGRIP_CMD_ICS_MIN_MAX				(0x4335)
...
/* ------------------------------------------------------------------------- */
/* shgrip_cmd_ics_min_max                                                    */
/* ------------------------------------------------------------------------- */
static int shgrip_cmd_ics_min_max(unsigned char* data, int size)
{
	int ret = 0;
	
	SHGRIP_DBG("start\n");
	
	ret = shgrip_spi_transfer_write_block(SHGRIP_CMD_ICS_MIN_MAX, data, size);
	if (ret) {
		SHGRIP_ERR("SHGRIP_CMD_ICS_MIN_MAX failed. ret:%d\n", ret);
		return ret;
	}
...
static int shgrip_seq_reset_recovery(void)
{
	...
	ret = shgrip_cmd_ics_min_max(bk_adj.shgrip_ics_min_max, sizeof(bk_adj.shgrip_ics_min_max));
	if (ret) {
		SHGRIP_ERR("shgrip_cmd_ics_min_max err. ret=%d\n", ret);
		return GRIP_RESULT_FAILURE;
	}
...
static int shgrip_seq_set_sensor_adjust(struct shgrip_sens_setting_params *data)
{
	...
	ret = shgrip_cmd_ics_min_max(data->shgrip_ics_min_max, sizeof(data->shgrip_ics_min_max));
	if (ret) {
		SHGRIP_ERR("shgrip_cmd_ics_min_max err. ret=%d\n", ret);
		return GRIP_RESULT_FAILURE;
	}
...
static int shgrip_dev_spi_probe(struct spi_device *spi)
{
	...
	if (ret) {
		SHGRIP_ERR("GPIO(RST) REQUEST NG\n");
		goto shgrip_gpio_rst_err;
	}
