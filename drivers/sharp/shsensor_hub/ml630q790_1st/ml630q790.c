...
static uint32_t shub_err_code = 0;      /* SHMDS_HUB_0322_01 add */
...
/* SHMDS_HUB_0109_02 mod E */
        ret = shub_hostcmd(&cmd, &res, EXE_HOST_ALL, 1);
        if((SHUB_RC_OK != ret) || (0 != res.err.u16)) {
			shub_err_code = RESU16_SHUB_ERR_CODE(cmd,res);   /* SHMDS_HUB_0322_01 add */
...
/* SHMDS_HUB_0322_01 add S */
int32_t shub_get_error_code(void){
    uint32_t ret = 0;
...
// SHMDS_HUB_0319_01 mod S
    ...
    if(ret != SHUB_RC_OK) {
        ...
        DBG(DBG_LV_ERROR, "Failed shub_init_param. ret=%x\n", ret);
        //return ret;
    }
...
int32_t shub_update_fw(bool boot, uint8_t *arg_iDataPage1, uint8_t *arg_iDataPage2, uint32_t arg_iLen)
{
    ...
    if(ret != SHUB_RC_OK) {
		shub_err_code = SHUB_FUP_ERR_WRITE;          /* SHMDS_HUB_0322_01 add */
    ...
    if(ret != SHUB_RC_OK) {
        ...
        if(ret == SHUB_RC_ERR){
            shub_err_code = SHUB_FUP_ERR_CHECK_ACCESS;
            DBG(DBG_LV_ERROR, "%s : shub_initialize(check_access) Error( ret=%d )\n", __FUNCTION__, ret);
        } else{
            shub_err_code = SHUB_FUP_ERR_INIT_COMMON;
...
{
	SHUB_DGB_TIME_INIT     /* SHMDS_HUB_1801_01 add */
    HostCmd cmd;
    HostCmdRes res;
    int32_t ret=0;
