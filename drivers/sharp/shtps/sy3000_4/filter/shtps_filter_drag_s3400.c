		...
		if(info->fingers[i].state == SHTPS_TOUCH_STATE_FINGER /* info->fingers[i].state == SHTPS_TOUCH_STATE_PEN */){
			if(ts->report_info.fingers[i].state != SHTPS_TOUCH_STATE_NO_TOUCH){
				if(ts->drag_hist_p->drag_hist[i][xy].count >= SHTPS_DRAG_SMOOTH_COUNT_MIN){
					...
					if(SHTPS_DRAG_SMOOTH_USE_HULL && ts->drag_hist_p->drag_hist[i][xy].skip_comp){
						...
						if(xy == SHTPS_POSTYPE_X){
							...
							SHTPS_LOG_DRAG_SMOOTH("[DEBUG_SMOOTH_X]exclude drag_smooth : info->fingers[i].x=%d\n",info->fingers[i].x);
						}else{
							if(temp_FIXED < 0){
								info->fingers[i].y = 0;
								ts->drag_hist_p->drag_hist[i][xy].pre_comp_history_FIXED = 0;
							...
							SHTPS_LOG_DRAG_SMOOTH("[DEBUG_SMOOTH_Y]exclude drag_smooth : info->fingers[i].y=%d\n",info->fingers[i].y);
						}
...
static int shtps_is_low_frame_rate_mode(struct shtps_rmi_spi *ts)
{
		...
		if(SHTPS_CTRL_FW_REPORT_RATE_ENABLE){
			...
			#else
				if(ts->fw_report_rate_cur == SHTPS_CTRL_FW_REPORT_RATE_PARAM){
					return 1;
				}
			#endif
		}
