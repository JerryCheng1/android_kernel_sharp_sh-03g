...
/* -------------------------------------------------------------------------- */
static int shtps_fwctl_s3400_initparam_transition_tuning(struct shtps_fwctl_info *fc_p)
{
	SHTPS_LOG_FWCTL_FUNC_CALL();
	#if defined(SHTPS_CHANGE_TRANSITION_TUNING_ENABLE)
		{
			...
			u8 buf[8];
				...
				fc_p->map_p->reg_F12_CTRL11_val[2] = ( (SHTPS_REG_PRM_ENABLE_LAND_LIFT << 0) |
													   (SHTPS_REG_PRM_ENABLE_LAND_CONDITION << 1) |
													   (SHTPS_REG_PRM_ENABLE_LIFT_CONDITION << 2) );
				fc_p->map_p->reg_F12_CTRL11_val[3] = SHTPS_REG_PRM_LAND_W_THRESHOLD;
				fc_p->map_p->reg_F12_CTRL11_val[4] = SHTPS_REG_PRM_LAND_Z_THRESHOLD;
				fc_p->map_p->reg_F12_CTRL11_val[5] = SHTPS_REG_PRM_BUFFERING_DISTANCE;
				fc_p->map_p->reg_F12_CTRL11_val[7] = SHTPS_REG_PRM_LAND_LOCK_DISTANCE;
			#else
				rc = M_READ_FUNC(fc_p, fc_p->map_p->fn12.ctrl.num[11].addr, buf, sizeof(buf));
			#endif /* SHTPS_SPI_AVOID_BLOCKREAD_FAIL */

			buf[2] = ( (SHTPS_REG_PRM_ENABLE_LAND_LIFT << 0) |
					   (SHTPS_REG_PRM_ENABLE_LAND_CONDITION << 1) |
					   (SHTPS_REG_PRM_ENABLE_LIFT_CONDITION << 2) );
			buf[3] = SHTPS_REG_PRM_LAND_W_THRESHOLD;
			buf[4] = SHTPS_REG_PRM_LAND_Z_THRESHOLD;
			buf[5] = SHTPS_REG_PRM_BUFFERING_DISTANCE;
			buf[7] = SHTPS_REG_PRM_LAND_LOCK_DISTANCE;
			rc = M_WRITE_PACKET_FUNC(fc_p, fc_p->map_p->fn12.ctrl.num[11].addr, buf, sizeof(buf));
			return rc;
		}
	#else
...
struct shtps_fwctl_functbl		shtps_fwctl_s3400_function_table = {
		...
		.initparam_transition_tuning_f				= shtps_fwctl_s3400_initparam_transition_tuning,
