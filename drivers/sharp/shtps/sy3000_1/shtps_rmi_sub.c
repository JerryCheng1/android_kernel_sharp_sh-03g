 ...
 */
#if defined( SHTPS_DEF_RECORD_LOG_FILE_ENABLE )
	#define SHTPS_DURABLE_LOG_FILE			("/durable/touchpanel/touchpanel.log")
	#define SHTPS_DURABLE_LOG_FILE_CHECK_SIZE	5000
	#define SHTPS_DURABLE_EVENT_STORE_NUM		100
	#define SHTPS_DURABLE_DATE_STR_LENGTH		34
	#define SHTPS_DURABLE_EVENT_STR_LENGTH		100
	#define SHTPS_DURABLE_FRAMELINE_LOG_SIZE	3600
	#define SHTPS_DURABLE_LOG_BUF_SPACING_SIZE	(((SHTPS_DURABLE_EVENT_STORE_NUM * (SHTPS_DURABLE_DATE_STR_LENGTH + SHTPS_DURABLE_EVENT_STR_LENGTH))*2)+SHTPS_DURABLE_FRAMELINE_LOG_SIZE)

	struct shtps_durable_event_store_log {
		char	event_log_str[SHTPS_DURABLE_DATE_STR_LENGTH + SHTPS_DURABLE_EVENT_STR_LENGTH];
		int	event_log_length;
	};

	struct shtps_durable_log_info {
		int	write_length;
		int	read_flag;
		int	durable_file_out_flag;
		int	event_store_tbl_array;
		char	date_str_buf[SHTPS_DURABLE_DATE_STR_LENGTH];
		char	write_buf[SHTPS_DURABLE_LOG_FILE_CHECK_SIZE+128];
		struct shtps_durable_event_store_log 	event_store_tbl[SHTPS_DURABLE_EVENT_STORE_NUM];
	};
	static struct shtps_durable_log_info *shtps_dur_info_p = 0;
	...
	int shtps_durable_api_add_log(int type);
	void shtps_durable_api_memory_init(void);
	void shtps_durable_api_memory_deinit(void);
	static ssize_t shtps_durable_kernel_write(struct file *fp, const char *buf, size_t size);
	static ssize_t shtps_durable_kernel_read(struct file *fp, char *buf, size_t size , unsigned int offset);
	static int shtps_durable_log_file_size_check(char *buf, size_t check_size);
	static int shtps_durable_file_init_read(void);
	static int shtps_durable_file(void);
	static int shtps_durable_summary_init_file(void);
	static void durable_get_date_str(char *str_buf);
	static int shtps_durable_add_event_log_memory(char *logstr);
	static int shtps_durable_add_event_log_store(char *logstr, struct shtps_durable_event_store_log *store_tbl_p, int *array);
	static int shtps_durable_event_log_buf(void);
#endif /* SHTPS_DEF_RECORD_LOG_FILE_ENABLE */
...
static void shtps_perflock_enable(struct shtps_rmi_spi *ts)
{
	...
	shtps_mutex_lock_cpu_clock_ctrl();
		...
		if(SHTPS_CTRL_CPU_CLOCK_FOR_LOW_BRIGHTNESS_ENABLE == 0){
			if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) != 0 ){
				SHTPS_LOG_DBG_PRINT("perf_lock not start because lcd bright is low.\n");
				req = 0;
			}
		}
		...
		if(SHTPS_CTRL_CPU_CLOCK_FOR_ECO_ENABLE == 0){
			if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_ECO) != 0 ){
				SHTPS_LOG_DBG_PRINT("perf_lock not start because eco mode is on.\n");
				req = 0;
			}
		}
	...
	if(req == 1){
		ts->cpu_clock_ctrl_p->perf_lock_state = 1;
	...
	if(SHTPS_PERF_LOCK_QOS_LATENCY_ENABLE != 0){
		if(ts->cpu_clock_ctrl_p->qos_cpu_latency_state == 0){
			pm_qos_update_request(&ts->cpu_clock_ctrl_p->qos_cpu_latency, SHTPS_PERF_LOCK_QOS_LATENCY_DEF_VALUE);
			ts->cpu_clock_ctrl_p->qos_cpu_latency_state = 1;
			SHTPS_LOG_DBG_PRINT("prohibit Standalone Powercollapse start\n");
		}
	}
...
static void shtps_perflock_disable(struct shtps_rmi_spi *ts)
{
	...
	if(ts->cpu_clock_ctrl_p->perf_lock_state != 0){
		for_each_possible_cpu(cpu) {
			if (cpu <= 3) {
		...
		sh_cpufreq_update_policy_try();
		ts->cpu_clock_ctrl_p->perf_lock_state = 0;
		SHTPS_LOG_DBG_PRINT("perf_lock end\n");
	}

	if(ts->cpu_clock_ctrl_p->qos_cpu_latency_state == 1){
		pm_qos_update_request(&ts->cpu_clock_ctrl_p->qos_cpu_latency, PM_QOS_DEFAULT_VALUE);
		ts->cpu_clock_ctrl_p->qos_cpu_latency_state = 0;
		SHTPS_LOG_DBG_PRINT("prohibit Standalone Powercollapse end\n");
	}
...
int shtps_durable_api_add_log(int type)
{
	...
	if(shtps_dur_info_p == NULL) {
		PR_ERROR("durable log info not allocated :%s()\n", __func__);
		return ret;
	}

	if(shtps_dur_info_p->durable_file_out_flag == 1) {
        	SHTPS_RECORD_LOG_FILE_PRINT("%s()  status is durable file out \n", __func__);
	}
	
	ret = shtps_durable_file_init_read();
	if(ret == SHTPS_DURABLE_RESULT_FAILURE) {
		return ret;
	}

	switch(type) {
	case SHTPS_DURABLE_LOG_TYPE_LCD_ON:
		sprintf(logstr, "LCD_ON\n");
		break;
	case SHTPS_DURABLE_LOG_TYPE_LCD_OFF:
		sprintf(logstr, "LCD_OFF\n");
		break;
	case SHTPS_DURABLE_LOG_TYPE_REZERO:
		sprintf(logstr, "REZERO\n");
		break;
	}
	
	shtps_durable_add_event_log_memory(logstr);
	ret = shtps_durable_event_log_buf();
	ret = shtps_durable_file();
...
void shtps_durable_api_memory_init(void)
{
	shtps_dur_info_p = kzalloc(sizeof(struct shtps_durable_log_info), GFP_KERNEL);
	if(shtps_dur_info_p == NULL){
		PR_ERROR("durable log info memory allocation error:%s()\n", __func__);
		return;
	}

	memset(shtps_dur_info_p, 0x00, sizeof(struct shtps_durable_log_info));
	shtps_dur_info_p->write_length = 0;
	shtps_dur_info_p->read_flag = 0;
	shtps_dur_info_p->durable_file_out_flag = 0;
...
static int shtps_durable_log_file_size_check(char *buf, size_t check_size)
{
	...
	int k;
	char *move_point;
	if((shtps_dur_info_p->write_length + check_size) >= SHTPS_DURABLE_LOG_FILE_CHECK_SIZE) {
		for(i=SHTPS_DURABLE_LOG_BUF_SPACING_SIZE;i<SHTPS_DURABLE_LOG_FILE_CHECK_SIZE;i++) {
			if(buf[i] == '/') {
				if((buf[i+3] == '/') && (buf[i+6] == '(')) {
	...
	return SHTPS_DURABLE_RESULT_SUCCESS;
}
static int shtps_durable_file_init_read(void)
{
	struct path  path;
	...
	if(shtps_dur_info_p->read_flag == 1) {
		SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file already init read\n", __func__);
		return SHTPS_DURABLE_RESULT_SUCCESS;
	}
	ret = kern_path(SHTPS_DURABLE_LOG_FILE, LOOKUP_OPEN, &path);
	if (ret != 0) {
		ret = shtps_durable_summary_init_file();
		if (ret != SHTPS_DURABLE_RESULT_SUCCESS) {
			return SHTPS_DURABLE_RESULT_FAILURE;
		}
	}
	SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file open start\n", __func__);
	fp = filp_open(SHTPS_DURABLE_LOG_FILE, O_RDWR, 0660);
	if (IS_ERR_OR_NULL(fp)) {
		SHTPS_RECORD_LOG_FILE_PRINT("%s()  Cannot open file: %s\n", __func__, SHTPS_DURABLE_LOG_FILE);
		return SHTPS_DURABLE_RESULT_FAILURE;
	}
	SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file open end\n", __func__);

	SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file read start\n", __func__);
	memset(shtps_dur_info_p->write_buf, 0x00, sizeof(shtps_dur_info_p->write_buf));
	shtps_dur_info_p->write_length = shtps_durable_kernel_read(fp, shtps_dur_info_p->write_buf, sizeof(shtps_dur_info_p->write_buf), 0);
	shtps_dur_info_p->read_flag = 1;
	SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file read end  length=%d\n", __func__, shtps_dur_info_p->write_length);
	...
	if(event_log_str_cnt >= SHTPS_DURABLE_EVENT_STORE_NUM) {
		event_log_str_cnt = 0;
	}
...
static int shtps_durable_file(void)
{
	...
	if(shtps_dur_info_p->write_length) {
		SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file open start\n", __func__);
		fp = filp_open(SHTPS_DURABLE_LOG_FILE, O_RDWR|O_TRUNC, 0660);
		if (IS_ERR_OR_NULL(fp)) {
			SHTPS_RECORD_LOG_FILE_PRINT("%s()  Cannot open file: %s\n", __func__, SHTPS_DURABLE_LOG_FILE);
			return SHTPS_DURABLE_RESULT_FAILURE;
		}
		...
		shtps_dur_info_p->write_length = shtps_durable_kernel_write(fp, shtps_dur_info_p->write_buf, shtps_dur_info_p->write_length);
		SHTPS_RECORD_LOG_FILE_PRINT("%s()  write_length: %d write end \n", __func__, shtps_dur_info_p->write_length);

		SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file close start\n", __func__);
		filp_close(fp, NULL);
		SHTPS_RECORD_LOG_FILE_PRINT("%s()  durable file close end\n", __func__);
	}else{
	}
...
static int shtps_durable_summary_init_file(void)
{
	...
	SHTPS_RECORD_LOG_FILE_PRINT("%s() open file: %s pid=%d tgid=%d comm=%s\n", __func__, SHTPS_DURABLE_LOG_FILE, current->pid, current->tgid,
                                                                                                       current->comm);
	fp = filp_open(SHTPS_DURABLE_LOG_FILE, O_WRONLY | O_CREAT | O_TRUNC, 0660);
	if (IS_ERR_OR_NULL(fp)) {
...
{
	int ret = 0;
	ret = shtps_durable_add_event_log_store(logstr, shtps_dur_info_p->event_store_tbl, &(shtps_dur_info_p->event_store_tbl_array));
	return ret;
}
...
static int shtps_durable_add_event_log_store(char *logstr, struct shtps_durable_event_store_log *store_tbl_p, int *array)
{
	...
	if(loglen > 0) {
		...
		durable_get_date_str(date_buf_p);
		...
		(*array)++;
		if(*array >= SHTPS_DURABLE_EVENT_STORE_NUM) {
			*array = 0;
		}
	}
...
static int shtps_durable_event_log_buf(void)
{
	...
	struct shtps_durable_event_store_log  *store_tbl_p;
	...
	for(i=0;i<SHTPS_DURABLE_EVENT_STORE_NUM;i++) {
		log_total_length += store_tbl_p[i].event_log_length;
	}
	shtps_durable_log_file_size_check(shtps_dur_info_p->write_buf, log_total_length);
	...
	if(wk_array == 0) {
		...
		for(i=wk_array;i<SHTPS_DURABLE_EVENT_STORE_NUM;i++) {
			if(store_tbl_p[i].event_log_length > 0) {
				memcpy(&(shtps_dur_info_p->write_buf[write_buf_cnt]), store_tbl_p[i].event_log_str, store_tbl_p[i].event_log_length);
				shtps_dur_info_p->write_length += store_tbl_p[i].event_log_length;
				write_buf_cnt += store_tbl_p[i].event_log_length;
	...
	while( list_empty( &(ts->queue) ) == 0 ){
		...
		switch(ts->cur_msg_p->event){
			...
			case SHTPS_FUNC_REQ_EVEMT_LCD_ON:
				#if defined( SHTPS_DEF_RECORD_LOG_FILE_ENABLE )
				shtps_durable_api_add_log(SHTPS_DURABLE_LOG_TYPE_LCD_ON);
			...
			case SHTPS_FUNC_REQ_EVEMT_LCD_OFF:
				#if defined( SHTPS_DEF_RECORD_LOG_FILE_ENABLE )
				shtps_durable_api_add_log(SHTPS_DURABLE_LOG_TYPE_LCD_OFF);
				#endif /* #if defined( SHTPS_DEF_RECORD_LOG_FILE_ENABLE ) */
...
void shtps_record_log_file_init(struct shtps_rmi_spi *ts)
{
	...
	shtps_durable_api_memory_init();   /* for durable */
}
...
void shtps_record_log_file_deinit(struct shtps_rmi_spi *ts)
{
	...
	shtps_durable_api_memory_deinit();   /* for durable */
}
