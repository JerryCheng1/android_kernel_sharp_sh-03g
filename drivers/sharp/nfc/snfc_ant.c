...
static ssize_t snfc_ant_read(struct file *file, char __user *buf, size_t len, loff_t *pos)
{
	...
	NFC_DRV_DBG_LOG("END data=0x%02X, len=%d", data[0], (int)len);
...
static const struct of_device_id snfc_ant_match_table[] = {
    { .compatible = D_SNFC_ANT_DEV_NAME, },
    {}
};
...
static struct i2c_driver s7760a_driver =
{
	.driver = {
		...
		.of_match_table = snfc_ant_match_table,
	},
