/* drivers/sharp/shdisp_lcdc/shdisp_pm.c  (Display Driver)
 *
...
void shdisp_pm_init(struct shdisp_boot_context *shdisp_boot_ctx)
{
    ...
    shdisp_pm_ctx.bdic_is_exist = shdisp_boot_ctx->bdic_is_exist;
    shdisp_pm_ctx.bdic_chipver  = shdisp_boot_ctx->bdic_chipver;

    shdisp_pm_ctx.psals_status.power_status = SHDISP_DEV_STATE_NOINIT;
    shdisp_pm_ctx.psals_status.sensor_state = SHDISP_SENSOR_STATE_PROX_OFF_ALS_OFF;
...
int shdisp_pm_bdic_power_manager(int type, int state)
{
    ...
    if (shdisp_pm_ctx.bdic_is_exist != SHDISP_BDIC_IS_EXIST) {
...
int shdisp_pm_psals_power_manager(int type, int state)
{
    ...
    if (shdisp_pm_ctx.bdic_status.power_status == SHDISP_DEV_STATE_OFF) {
        SHDISP_ERR("cannot sensor %s because bdic power off", ((state == SHDISP_DEV_STATE_OFF) ? "off" : "on"));
        return SHDISP_RESULT_FAILURE;
    }
#endif  /* SHDISP_USE_LEDC */
...
static int shdisp_pm_psals_ps_init(void)
{
    ...
    case SHDISP_SENSOR_STATE_PROX_ON_ALS_ON:
        break;
    default:
        SHDISP_ERR("<RESULT_FAILURE> invalid state %d.", state);
