/* drivers/sharp/shdisp_lcdc/shdisp_system.c  (Display Driver)
 *
...
#define SHDISP_CLMR_SPI_WRITE_SPEED             48000000
...
static int shdisp_clmr_sio_probe(struct spi_device *spi);
static int shdisp_clmr_sio_remove(struct spi_device *spi);
...
static struct spi_driver clmr_spi_driver = {
    .driver = {
        .name = "clmr_spi",
        .owner = THIS_MODULE,
...
int  shdisp_SYS_Host_control(int cmd, unsigned long rate)
{
    ...
    SHDISP_TRACE("in cmd=%d, rate=%08lx", cmd, rate);
    switch (cmd) {
    ...
    case SHDISP_HOST_CTL_CMD_LCD_CLK_EXIT:
        ...
        break;
    default:
        SHDISP_ERR("<INVALID_VALUE> cmd(%d), rate(%d).", cmd, (int)rate);
...
void shdisp_SYS_delay_us(unsigned long usec)
{
    ...
    if (ret == -ERESTART_RESTARTBLOCK) {
        ...
        SHDISP_WARN("msleep start ret=%d wait_ms=%u", ret, wait_ms);
        msleep(wait_ms);
...
int shdisp_SYS_Host_gpio_request(int num, char *label)
{
...
int  shdisp_SYS_set_irq( int enable )
{
    ...
    if (enable == SHDISP_IRQ_ENABLE) {
        ...
        SHDISP_ERR("<INVALID_VALUE> shdisp_SYS_set_irq:enable=%d.", enable);
...
int  shdisp_SYS_bdic_i2c_read(unsigned char addr, unsigned char *data)
{
    ...
    *data = rtnbuf[0];
    SHDISP_DEBUG("[SHDISP]%s addr=0x%02x, data=0x%02x", __func__, addr, *data);
    return SHDISP_RESULT_SUCCESS;
}
...
int  shdisp_SYS_bdic_i2c_multi_read(unsigned char addr, unsigned char *data, int size)
{
    ...
    if (shdisp_FWCMD_buf_get_nokick()) {
        ...
        for (i = 0; i < size; i++) {
            ...
            *data = rtnbuf[0];
            SHDISP_DEBUG("addr=0x%02x, data=0x%02x", ary[1], *data);
            ary[1]++;
            data++;
        }
    } else {
...
int  shdisp_SYS_psals_i2c_read(unsigned char addr, unsigned char *data)
{
    ...
    *data = rtnbuf[0];
    SHDISP_TRACE("addr=0x%02x, data=0x%02x", addr, *data);
    return SHDISP_RESULT_SUCCESS;
}
...
int  shdisp_SYS_clmr_sio_init(void)
{
    ...
    ret = spi_register_driver(&clmr_spi_driver);
    if (ret < 0) {
...
{
    spi_unregister_driver(&clmr_spi_driver);
...
int  shdisp_SYS_clmr_sio_transfer(unsigned short reg, unsigned char *wbuf, int wlen, unsigned char *rbuf, int rlen)
{
    ...
    if (wbuf) {
        ...
        *(int*)(spidata) = *(int*)(wbuf);
        ...
        if (ret < 0) {
            SHDISP_ERR(":[%d] spi_sync(write register and addr set) ret=%d ", __LINE__, ret);
        ...
        *spidata = 0xC4;
        ...
        SHDISP_PERFORMANCE_DEBUG("COMMON SPI-TRANSFER-PROCESS 0010 END");
        if (ret < 0) {
            SHDISP_ERR(":[%d] spi_sync(read register data) ret=%d ", __LINE__, ret);
            result = SHDISP_RESULT_FAILURE;
        }
        else {
            result = SHDISP_RESULT_SUCCESS;
    ...
    if (mode & 1) {
        ...
        if (!(len < 16)) {
            ...
            *spi_transfer_buf = (mode & 2) ? 0xCA : 0xC9;
            ...
            SHDISP_PERFORMANCE_DEBUG("COMMON SPI-TRANSFER-PROCESS 0010 END");
            if (ret < 0) {
                SHDISP_ERR(":[%d] spi_sync() ret=%d ", __LINE__, ret);
                result = SHDISP_RESULT_FAILURE;
            }
...
static void shdisp_host_ctl_lcd_clk_start(unsigned long rate)
{
    ...
    if (ret) {
        SHDISP_ERR("%s failed to vote for TCXOA1. ret=%d", __func__, ret);
    } else {
    ...
    ret = msm_xo_mode_vote(xo_handle , MSM_XO_MODE_ON);
    if (ret) {
        SHDISP_ERR("%s failed to vote for TCXOD1. ret=%d", __func__, ret);
    }
...
static int shdisp_host_ctl_lcd_clk_init()
{
    ...
    if (IS_ERR(xo_handle)) {
        ...
        SHDISP_ERR("%s not able to get the handle to vote for TCXO A1 buffer. ret=%d", __func__, ret);
        ret = SHDISP_RESULT_FAILURE;
    }
#elif defined(CONFIG_SHDISP_EXCLK_PMIC_D1)
    ...
    if (IS_ERR(xo_handle)) {
        ret = PTR_ERR(xo_handle);
        SHDISP_ERR("%s not able to get the handle to vote for TCXO D1 buffer. ret=%d", __func__, ret);
        ret = SHDISP_RESULT_FAILURE;
    }
#endif  /* CONFIG_SHDISP_EXCLK_*** */
...
unsigned short shdisp_sys_api_hw_revision_bit_conv(unsigned short hw_revision_bit)
{
    ...
    case SHDISP_HW_REV_BIT_RESERVE1:
        ret = SHDISP_HW_REV_ES15;
        break;

    case SHDISP_HW_REV_BIT_RESERVE2:
        ret = SHDISP_HW_REV_PP15;
        break;
...
int shdisp_SYS_variable_length_data_read(unsigned char cmdno, unsigned char *data, int size)
{
    ...
    do {
        ...
        if ((check & 0xC0000000) == 0x40000000) {
            ...
            SHDISP_DEBUG("ARMINFOREG2[0-3] = 0x%02x%02x%02x%02x", arminfo[0], arminfo[1], arminfo[2], arminfo[3]);
            SHDISP_DEBUG("ARMINFOREG3[4-7] = 0x%02x%02x%02x%02x", arminfo[4], arminfo[5], arminfo[6], arminfo[7]);
            ...
            if ((check & 0xC0000000) == 0x80000000) {
                ...
                SHDISP_DEBUG("ARMINFOREG2[0-3] = 0x%02x%02x%02x%02x", arminfo[0], arminfo[1],
                                                                        arminfo[2], arminfo[3]);
                SHDISP_DEBUG("ARMINFOREG3[4-7] = 0x%02x%02x%02x%02x", arminfo[4], arminfo[5],
                                                                        arminfo[6], arminfo[7]);
...
static int shdisp_SYS_getInfoReg0_check(int mode)
{
    ...
    if (i >= SHDISP_FW2HOST_WAIT_COUNT) {
        SHDISP_ERR(":[%d] timeout!!! mode=%d info[0]=0x%x", __LINE__, mode, info[0]);
    } else {
        SHDISP_DEBUG("mode = %d count = %d data = 0x%02x%02x%02x%02x", mode, i, info[0], info[1], info[2], info[3]);
    }
...
int shdisp_SYS_getInfoReg1(void)
{
    ...
    SHDISP_DEBUG("data = 0x%02x%02x%02x%02x", info[0], info[1], info[2], info[3]);
    return (info[0] << 24) | (info[1] << 16) | (info[2] << 8) | info[3];
}
...
int shdisp_SYS_getInfoReg2(void)
{
    ...
    SHDISP_DEBUG("data = 0x%02x%02x%02x%02x", info[0], info[1], info[2], info[3]);
    SHDISP_DEBUG("ADO = %x", ado);
    SHDISP_DEBUG("ADO_TBL = %x", ado_tbl);
    SHDISP_DEBUG("ADO_CND = %x", ado_cnd);
#endif   /* SHDISP_LOG_ENABLE */
...
char shdisp_FWCMD_set_apino(unsigned char apino)
{
    ...
    if (apino <= SHDISP_CLMR_FWCMD_APINO_MAX) {
        ...
        SHDISP_DEBUG("%s apino =0x%02x error", __func__, apino);
    }
...
int shdisp_FWCMD_safe_finishanddoKick( void )
{
    ...
    &&  (!shdisp_FWCMD_buf_is_empty())) {
        ...
        result = SHDISP_RESULT_SUCCESS;
        SHDISP_DEBUG("%s buffer is empty end. ", __func__);
    }
    return result;
}
...
static void shdisp_FWCMD_dump( unsigned char *str, unsigned char *begin, unsigned short len )
{
    ...
    if (!buf) {
        SHDISP_DEBUG( "str=%s, but begin == NULL end", str );
        return;
    }
...
int shdisp_FWCMD_doKick(unsigned char notice, unsigned int len, unsigned char *rtnbuf )
{
    ...
    &&  (eDramReadbuf)) {
        ...
        SHDISP_DEBUG( "dokickAndeDramRead compare=%d", eDramcompare );
...
int shdisp_FWCMD_eDramPtrReset(void)
{
    ...
    if (rtn == SHDISP_RESULT_FAILURE) {
        SHDISP_ERR("ptrrst:[%d] failed. ", __LINE__ );
        return SHDISP_RESULT_FAILURE;
    }

    rtn = shdisp_clmr_api_wait4eDramPtr_rst_comp();
    if (rtn == SHDISP_RESULT_FAILURE) {
        SHDISP_ERR("timeout:[%d] ", __LINE__);
        return SHDISP_RESULT_FAILURE;
    }
    return SHDISP_RESULT_SUCCESS;
}
...
void shdisp_gpio_config(int num, int func, int drive, int inout, int pull)
{
    ...
    SHDISP_DEBUG("[BEFORE] GPIO[%d],F[%d],DS[%s],IO[%d],PL[%d]", num, rfunc, DriveStr[rdrive], rinout, rpull);
#endif  /* SHDISP_GPIO_CONFIG_LOG_ENABLE */


#ifdef  SHDISP_GPIO_CONFIG_LOG_ENABLE
    shdisp_gpio_get_config(num, &rfunc, &rdrive, &rinout, &rpull);
    SHDISP_DEBUG("[AFTER] GPIO[%d],F[%d],DS[%s],IO[%d],PL[%d]", num, rfunc, DriveStr[rdrive], rinout, rpull);
#endif  /* SHDISP_GPIO_CONFIG_LOG_ENABLE */
    return;
}
...
{
    if (istimeout) {
        SHDISP_DEBUG("istimeout=%d ", istimeout );
    }
    return istimeout;
}
