/* drivers/sharp/shdisp_lcdc/shdisp_panel.c  (Display Driver)
 *
...
int shdisp_panel_API_check_panel_info(void)
{
    ...
    SHDISP_TRACE("in");
    SHDISP_TRACE("out revision=%04x", revision);
    return revision;
}
...
int shdisp_panel_API_check_flicker_param(unsigned short vcom_in, unsigned short *vcom_out)
{
    if (shdisp_panel_fops->check_flicker) {
        return shdisp_panel_fops->check_flicker(vcom_in, vcom_out);
    }
    return SHDISP_RESULT_SUCCESS;
}
...
int shdisp_panel_API_diag_set_flicker_param(struct shdisp_diag_flicker_param flicker_param)
{
    if (shdisp_panel_fops->set_flicker) {
        return shdisp_panel_fops->set_flicker(flicker_param);
    }
    return SHDISP_RESULT_SUCCESS;
}
...
int shdisp_panel_API_diag_get_flicker_param(struct shdisp_diag_flicker_param *flicker_param)
{
    if (shdisp_panel_fops->get_flicker) {
        return shdisp_panel_fops->get_flicker(flicker_param);
    }
    return SHDISP_RESULT_SUCCESS;
}
...
int shdisp_panel_API_diag_get_flicker_low_param(struct shdisp_diag_flicker_param *flicker_param)
{
    if (shdisp_panel_fops->get_flicker_low) {
        return shdisp_panel_fops->get_flicker_low(flicker_param);
    }
    return SHDISP_RESULT_SUCCESS;
}
...
{
    SHDISP_TRACE("in");
    SHDISP_TRACE("out type=%d", drive_freq_last_type);
...
static int shdisp_panel_mipi_dsi_dcs_lwrite(struct dsi_buf *dp, struct dsi_cmd_desc *cm)
{
    ...
    *hp |= (uint32)(SHDISP_DSI_HDR_DATA1(cm->dlen & 0x00FF));
    ...
    SHDISP_TRACE("out dp->len=%d", dp->len);
    return dp->len;
}
...
static int shdisp_panel_mipi_dsi_cmd_dma_add(struct dsi_buf *dp, struct dsi_cmd_desc *cm)
{
    ...
    default:
        SHDISP_DEBUG("dtype=%x NOT supported", cm->dtype);
        break;
    }
    return len;
}
...
int shdisp_panel_API_mipi_dsi_cmds_tx(struct dsi_buf *tp, struct dsi_cmd_desc *cmds, int cnt)
{
    ...
    */
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (cm->wait) {
            SHDISP_DEBUG("wait start=%d(ms)", cm->wait);
                                                                              ...
                                                                              unsigned char size)
{
    ...
    SHDISP_TRACE("out rp->len:%d, rlen:%d", rp->len, rlen);
...
unsigned char shdisp_panel_API_get_fw_transfer_mode_w(int dtype, int dsi_mode)
{
    ...
    default:
        fwcmd = SHDISP_CLMR_FWCMD_DSI_LP_SWRITE;
        SHDISP_ERR("unexpeted dsi_transfer_mode=%x", dsi_mode);
        break;
    }
    return fwcmd;
}
