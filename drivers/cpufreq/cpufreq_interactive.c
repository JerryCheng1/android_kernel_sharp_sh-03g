...
#define DEFAULT_FREQ_STEP									33
#define DEFAULT_FREQ_STEP_HIGH								5
#define DEFAULT_FREQUENCY_THRESHOLD_ONDEMAND_STEPHIGH		99
#define DEFAULT_FREQUENCY_THRESHOLD_OVER_ONDEMAND_STEPHIGH	124
#define DEFAULT_CPUFREQ_INTERACTIVE_THRESHOLD				960000
#define DEFAULT_CPUFREQ_INTERACTIVE_STEP					960000
#define DEFAULT_CPUFREQ_INTERACTIVE_STEPHIGH				960000
#define DEFAULT_CPUFREQ_INTERACTIVE_SWITCH_STEP				1248000
...
#define DEFAULT_TIMER_SLACK (4 * DEFAULT_TIMER_RATE)
	...
	unsigned int interactive_threshold;
	unsigned int interactive_step_freq;
	unsigned int interactive_step_high_freq;
	unsigned int interactive_switch_step_freq;
#endif /* CONFIG_SHSYS_CUST */
};
...
#ifdef CONFIG_SHSYS_CUST
struct sh_cpufreq_adjust_param sh_cpufreq_adjust = {
	.mdp_clk_ena = 0,
	.kgsl_is_working = false,
	.music_type = 0,
};
#endif /*CONFIG_SHSYS_CUST*/
...
#ifdef CONFIG_SHSYS_CUST
		if (pcpu->policy->cur < tunables->interactive_switch_step_freq) {
			if (pcpu->policy->cur < tunables->interactive_threshold) {
				new_freq = tunables->interactive_step_freq;
			} else {
				int inc = (pcpu->policy->cpuinfo.max_freq * tunables->freq_step) / 100;
				new_freq = pcpu->policy->cur + inc;
			}
			if (pcpu->policy->cur < tunables->interactive_step_high_freq && new_freq > tunables->interactive_step_high_freq) {
				if (cpu_load > tunables->up_threshold_freq_step_high) {
					new_freq = tunables->interactive_step_high_freq;
				} else {
					new_freq = pcpu->policy->cur;
				}
			} else if (pcpu->policy->cur >= tunables->interactive_step_high_freq) {
				if (cpu_load > tunables->up_threshold_over_freq_step_high) {
					int inc = (pcpu->policy->cpuinfo.max_freq * tunables->freq_step_high) / 100;
					new_freq = pcpu->policy->cur + inc;
				} else {
					new_freq = pcpu->policy->cur;
				}
...
#ifdef CONFIG_SHSYS_CUST
	sh_cpufreq_notify_adjust(&sh_cpufreq_adjust, SH_CPUFREQ_PERIOD);
	if (sh_cpufreq_adjust.mdp_clk_ena == 0 && sh_cpufreq_adjust.kgsl_is_working == false && sh_cpufreq_adjust.music_type == 0) {
		pr_debug("cpu%d : mdp_clk = %d gpu_clk = %d is_music_play = %d check clock up elapsed time\n", smp_processor_id(), sh_cpufreq_adjust.mdp_clk_ena, sh_cpufreq_adjust.kgsl_is_working, sh_cpufreq_adjust.music_type);
#endif /*CONFIG_SHSYS_CUST*/
	if (pcpu->policy->cur >= tunables->hispeed_freq &&
	    new_freq > pcpu->policy->cur &&
...
#ifdef CONFIG_SHSYS_CUST
	if (sh_cpufreq_adjust.mdp_clk_ena == 1 || sh_cpufreq_adjust.kgsl_is_working == true || sh_cpufreq_adjust.music_type != 0) {
...
#ifdef CONFIG_SHSYS_CUST
#define shsys_param_attr_rw(file_name)												\
static ssize_t show_##file_name(struct cpufreq_interactive_tunables *tunables,		\
		char *buf)																	\
{																					\
	return snprintf(buf, PAGE_SIZE, "%u\n", tunables->file_name);					\
}																					\
																					\
static ssize_t store_##file_name(struct cpufreq_interactive_tunables *tunables,		\
		const char *buf, size_t count)												\
{																					\
	int ret;																		\
	unsigned long val;																\
...
shsys_param_attr_rw(freq_step);
shsys_param_attr_rw(freq_step_high);
shsys_param_attr_rw(up_threshold_freq_step_high);
shsys_param_attr_rw(up_threshold_over_freq_step_high);
shsys_param_attr_rw(interactive_threshold);
shsys_param_attr_rw(interactive_step_freq);
shsys_param_attr_rw(interactive_step_high_freq);
shsys_param_attr_rw(interactive_switch_step_freq);
#endif /* CONFIG_SHSYS_CUST */
/*
...
show_store_gov_pol_sys(freq_step_high);
show_store_gov_pol_sys(up_threshold_freq_step_high);
show_store_gov_pol_sys(up_threshold_over_freq_step_high);
show_store_gov_pol_sys(interactive_threshold);
show_store_gov_pol_sys(interactive_step_freq);
show_store_gov_pol_sys(interactive_step_high_freq);
show_store_gov_pol_sys(interactive_switch_step_freq);
#endif /* CONFIG_SHSYS_CUST */
...
gov_sys_pol_attr_rw(freq_step_high);
gov_sys_pol_attr_rw(up_threshold_freq_step_high);
gov_sys_pol_attr_rw(up_threshold_over_freq_step_high);
gov_sys_pol_attr_rw(interactive_threshold);
gov_sys_pol_attr_rw(interactive_step_freq);
gov_sys_pol_attr_rw(interactive_step_high_freq);
gov_sys_pol_attr_rw(interactive_switch_step_freq);
#endif /* CONFIG_SHSYS_CUST */
...
static struct attribute *interactive_attributes_gov_sys[] = {
	...
	&freq_step_high_gov_sys.attr,
	&up_threshold_freq_step_high_gov_sys.attr,
	&up_threshold_over_freq_step_high_gov_sys.attr,
	&interactive_threshold_gov_sys.attr,
	&interactive_step_freq_gov_sys.attr,
	&interactive_step_high_freq_gov_sys.attr,
	&interactive_switch_step_freq_gov_sys.attr,
#endif /* CONFIG_SHSYS_CUST */
	NULL,
};
...
static struct attribute *interactive_attributes_gov_pol[] = {
	...
	&freq_step_high_gov_pol.attr,
	&up_threshold_freq_step_high_gov_pol.attr,
	&up_threshold_over_freq_step_high_gov_pol.attr,
	&interactive_threshold_gov_pol.attr,
	&interactive_step_freq_gov_pol.attr,
	&interactive_step_high_freq_gov_pol.attr,
	&interactive_switch_step_freq_gov_pol.attr,
#endif /* CONFIG_SHSYS_CUST */
	NULL,
};
...
#ifdef CONFIG_SHSYS_CUST
	tunables->freq_step = DEFAULT_FREQ_STEP;
	tunables->freq_step_high = DEFAULT_FREQ_STEP_HIGH;
	tunables->up_threshold_freq_step_high = DEFAULT_FREQUENCY_THRESHOLD_ONDEMAND_STEPHIGH;
	tunables->up_threshold_over_freq_step_high = DEFAULT_FREQUENCY_THRESHOLD_OVER_ONDEMAND_STEPHIGH;
	tunables->interactive_threshold = DEFAULT_CPUFREQ_INTERACTIVE_THRESHOLD;
	tunables->interactive_step_freq = DEFAULT_CPUFREQ_INTERACTIVE_STEP;
	tunables->interactive_step_high_freq = DEFAULT_CPUFREQ_INTERACTIVE_STEPHIGH;
	tunables->interactive_switch_step_freq = DEFAULT_CPUFREQ_INTERACTIVE_SWITCH_STEP;
#endif /* CONFIG_SHSYS_CUST */
