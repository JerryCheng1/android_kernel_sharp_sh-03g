...
void qpnp_adc_set_vbatt_calibration_data(void)
{
	...
	sh_calib_vbatt_vmax = cal.vmax;
}
EXPORT_SYMBOL(qpnp_adc_set_vbatt_calibration_data);

#if VBATT_CALIB_TYPE == 0
		...
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	pr_debug("[TEST] qpnp_adc_scale_vbatt amin = %d, amax = %d vmin = %d vmax = %d \n", sh_calib_vbatt_amin, sh_calib_vbatt_amax , sh_calib_vbatt_vmin , sh_calib_vbatt_vmax);
